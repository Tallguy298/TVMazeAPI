...
Func _GetShowInfo($ShowID, $ConvertTimeZone = 0, $SaveShowFiles = 0, $SaveShowFilesPath = @TempDir, $GetShowImage = 0, $SaveShowImagePath = @TempDir)
	...
	If $SaveShowFiles = 0 Then
		Local $SeriesInfo_EpisodeList = _INetGet("http://api.tvmaze.com/shows/" & $ShowID & "?embed=episodes")
		If @error Then Return SetError(1)
	Else
		DirCreate($SaveShowFilesPath)
		If Not FileExists($SaveShowFilesPath & "\" & $ShowID & ".dat") Then
			InetGet("http://api.tvmaze.com/shows/" & $ShowID & "?embed=episodes", $SaveShowFilesPath & "\" & $ShowID & ".dat", 1, 0)
			If @error Then Return SetError(1)
		EndIf
	...
	Local $Series_EpisodeList = _Between($SeriesInfo_EpisodeList, '"episodes":', "")
	If Not @error Then $Series_EpisodeList = $Series_EpisodeList[0]
	Local $SeriesInfo = _Between($SeriesInfo_EpisodeList, "", '"episodes":')
